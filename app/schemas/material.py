...
class MaterialUpload(BaseModel):
    ...
    def validate_file_type(cls, v: MaterialTypeEnum) -> MaterialTypeEnum:
        """Validate file type."""
        allowed = [MaterialTypeEnum.PDF, MaterialTypeEnum.PPTX, MaterialTypeEnum.DOC, MaterialTypeEnum.DOCX, MaterialTypeEnum.LINK]
        if v not in allowed:
            raise ValueError(f'Invalid file type. Allowed: {[m.value for m in allowed]}')
        return v
    
    @validator('file_url')
    def validate_file_url(cls, v: Optional[str], values: dict) -> Optional[str]:
        """Validate file URL."""
        if v is None or v == "":
            return v
        if not v.startswith(('http://', 'https://')):
            raise ValueError('File URL must be a valid URL')
        return v
    ...
    @validator('file_url', 'link_url', always=True)
    def validate_file_or_link(cls, v: Optional[str], values: dict) -> Optional[str]:
        """Ensure either file_url or link_url is provided."""
        file_type = values.get('file_type')
        file_url = values.get('file_url')
        link_url = values.get('link_url')
        
        if file_type == MaterialTypeEnum.LINK:
            if not link_url:
